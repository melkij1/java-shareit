...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    List<Booking> findAllByBookerIdAndStatus(long bookerId, BookingStatusEnum bookingStatus, Sort start);
    ...
    @Query("SELECT b FROM Booking b " +
            "WHERE b.item.owner.id = ?1 " +
            "AND b.status = ?2")
    List<Booking> findAllByOwnerIdAndStatus(long ownerId, BookingStatusEnum bookingStatus, Sort start);
